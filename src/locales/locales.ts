...
import type { TMetadata } from "@App/app/repo/scripts";
...
export function i18nName(script: { name: string; metadata: TMetadata }) {
  const m = script.metadata[`name:${i18n.language.toLowerCase()}`];
  return m ? m[0] : script.name;
}

export function i18nDescription(script: { metadata: TMetadata }) {
